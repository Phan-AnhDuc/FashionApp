...
public class VideoTimeBarForCameraSJ extends ScalableScaleBarForCameraSJ implements ScalableScaleBarForCameraSJ.TickMarkStrategy {

    private static final String TAG = "RSVideoTimeBar";
    public static final String MODE_UNIT_1_MIN = "scale unit 1 minute";
    public static final String MODE_UNIT_5_MIN = "scale unit 5 minute";
    public static final String MODE_UNIT_10_MIN = "scale unit 10 minute";
    public static final String MODE_UNIT_30_MIN = "scale unit 30 minute";
    public static final String MODE_UNIT_1_HOUR = "scale unit 1 hour";

    @StringDef({MODE_UNIT_1_MIN, MODE_UNIT_5_MIN, MODE_UNIT_10_MIN, MODE_UNIT_30_MIN, MODE_UNIT_1_HOUR})
    public @interface Mode {
    }

    private @Mode
    String mMode = MODE_UNIT_1_MIN;

    public static final long MODE_UNIT_1_MIN_VALUE = 30 * 60 * 1000;
    public static final long MODE_UNIT_5_MIN_VALUE = 1 * 60 * 60 * 1000;
    public static final long MODE_UNIT_10_MIN_VALUE = 2 * 60 * 60 * 1000;
    public static final long MODE_UNIT_30_MIN_VALUE = 8 * 60 * 60 * 1000;
    public static final long MODE_UNIT_1_HOUR_VALUE = 16 * 60 * 60 * 1000;
    ...
    public VideoTimeBarForCameraSJ(Context context) {
        this(context, null);
    }

    public VideoTimeBarForCameraSJ(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setRange(long start, long end) {
        ...
        if (!StringUtils.equals(mMode, MODE_UNIT_1_MIN)) {
            setMode(mMode, true);
        }
    }

    void setMode(@Mode String m, boolean setScaleRatio) {
        if (StringUtils.equals(mMode, m)) {
//            Log.e(TAG, " same mode.");
            return;
        }
        long spanValue;
        switch (m) {
            case MODE_UNIT_1_MIN:
                this.mMode = m;
                updateScaleInfo(5 * 60 * 1000, 1 * 60 * 1000);
                spanValue = MODE_UNIT_1_MIN_VALUE;
                break;
            case MODE_UNIT_5_MIN:
                this.mMode = m;
                updateScaleInfo(10 * 60 * 1000, 5 * 60 * 1000);
                spanValue = MODE_UNIT_5_MIN_VALUE;
                break;
            case MODE_UNIT_10_MIN:
                this.mMode = m;
                updateScaleInfo(30 * 60 * 1000, 10 * 60 * 1000);
                spanValue = MODE_UNIT_10_MIN_VALUE;
                break;
            case MODE_UNIT_30_MIN:
                this.mMode = m;
                updateScaleInfo(1 * 60 * 60 * 1000, 30 * 60 * 1000);
                spanValue = MODE_UNIT_30_MIN_VALUE;
                break;
            case MODE_UNIT_1_HOUR:
                this.mMode = m;
                updateScaleInfo(2 * 60 * 60 * 1000, 1 * 60 * 60 * 1000);
                spanValue = MODE_UNIT_1_HOUR_VALUE;
                break;
            default:
...
//        Log.e("TAG", " mode: " + mMode);
    ...
    protected void updateMode(float screenSpanValue) {
        if (screenSpanValue > MODE_UNIT_1_HOUR_VALUE) {
            setMode(MODE_UNIT_1_HOUR, false);
        } else if (screenSpanValue > MODE_UNIT_30_MIN_VALUE) {
            setMode(MODE_UNIT_30_MIN, false);
        } else if (screenSpanValue > MODE_UNIT_10_MIN_VALUE) {
            setMode(MODE_UNIT_10_MIN, false);
        } else if (screenSpanValue > MODE_UNIT_5_MIN_VALUE) {
            setMode(MODE_UNIT_5_MIN, false);
        } else {
